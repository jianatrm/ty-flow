...
const init = ()=>{
  ...
  state.graph.saveXML = (createFile = true) => exportXML(state.graph.save(),state.processModel,createFile);
  state.graph.saveImg = (createFile = true) => exportImg(canvasRef.value,state.processModel.name,createFile);

  state.graph.setMode(props.mode);
